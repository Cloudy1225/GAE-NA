...
from model import GAE_NA, GCNEncoder, SAGEEncoder, GATEncoder, GATv2Encoder
...
elif args.encoder == 'SGAE':
    encoder = SAGEEncoder(x.shape[1], hid_dim, out_dim)
elif args.encoder == 'GAT':
    encoder = GATEncoder(x.shape[1], out_dim)
elif args.encoder == 'GATv2':
    encoder = GATv2Encoder(x.shape[1], out_dim)
else:
    encoder = GCNEncoder(x.shape[1], hid_dim, out_dim)
